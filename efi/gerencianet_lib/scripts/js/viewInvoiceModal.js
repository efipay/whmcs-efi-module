window.onload = function () {
    ...
    const orderIsRecurring = $("#allItemsIsRecurring").val() == 1;
        ...
        if (i == 1) {
            ...
            if (brandOptionVerify && codSeguranca && mesVencimento && anoVencimento && numParcelas) {
                ...
                checkout.getPaymentToken({
                    ...
                    reuse: ($("#allItemsIsRecurring").val() == 1)
                }, (err, res) => {
                    if (err == null) {
    ...
    function verifyRecurring() {
        ...
        if (orderIsRecurring) {
            const paymentMethodsWithRecurring = ['Billet', 'Credit', 'Pix'];
            paymentMethodsWithRecurring.forEach(method => {
                $(`#subscriptionDiv${method}`).removeClass('d-none');
            });
        }
